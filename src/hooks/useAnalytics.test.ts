...
const DASHBOARD: AnalyticsDashboard = {
  ...
  totalUnmatched: 30,
  matchRate: 0.75,
  topQuestions: [{ faqId: 1, question: "Como troco?", category: "conta", count: 10 }],
  unanswered: [{ question: "Serve para chuva?", count: 3, lastAskedAt: "2026-01-01T10:00:00Z" }],
...
describe("useAnalytics", () => {
  ...
  it("fetches analytics for the default 14-day range and reshapes the view model", async () => {
    ...
    expect(result.current.data.topQuestions).toEqual([{ name: "Como troco?", count: 10 }]);
    expect(result.current.data.categories).toEqual([{ name: "conta", value: 40 }]);
  ...
  it("surfaces a friendly error message for non-auth failures", async () => {
    vi.mocked(getAnalytics).mockRejectedValue(new ApiError("Falha no servidor", 500));
    const { result } = renderHook(() => useAnalytics());
