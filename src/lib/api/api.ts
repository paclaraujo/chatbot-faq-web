...
export function askQuestion(question: string): Promise<ChatApiResponse> {
  return request<ChatApiResponse>("/chat", { method: "POST", body: { question } });
}
...
export type CreateFaqInput = { question: string; answer: string; category: string };
...
export function updateFaq(token: string, id: number, input: UpdateFaqInput): Promise<Faq> {
...
export function getAnalytics(token: string, params: AnalyticsParams = {}): Promise<AnalyticsDashboard> {
  const search = new URLSearchParams();
  if (params.topLimit != null) search.set("topLimit", String(params.topLimit));
  if (params.unansweredLimit != null) search.set("unansweredLimit", String(params.unansweredLimit));
  if (params.timelineDays != null) search.set("timelineDays", String(params.timelineDays));
  const qs = search.toString();
  return request<AnalyticsDashboard>(`/analytics${qs ? `?${qs}` : ""}`, { token });
}
