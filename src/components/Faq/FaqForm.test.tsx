...
describe("FaqForm", () => {
  it("shows the create heading and submit label when not editing", () => {
    ...
    expect(screen.getByRole("button", { name: /Cadastrar pergunta/ })).toBeInTheDocument();
  ...
  it("calls onChange with the updated field values", async () => {
    ...
    await user.type(screen.getByPlaceholderText("Como faço para trocar um produto?"), "O");
  ...
  it("calls onChange when editing the answer and category fields", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("Use **negrito** para destacar trechos importantes."),
      "R",
    );
  ...
  it("disables the submit button while saving", () => {
    setup({ saving: true });
    expect(screen.getByRole("button", { name: /Cadastrar pergunta/ })).toBeDisabled();
  });
