...
const FAQS: Faq[] = [
  { id: 1, question: "Como troco um produto?", answer: "Envie um e-mail.", category: "conta", createdAt: "2026-01-01" },
  { id: 2, question: "Funciona no escuro?", answer: "Sim.", category: "produto", createdAt: "2026-01-02" },
];
...
describe("useFaqAdmin", () => {
  ...
  it("submits a new entry, resets the form and refreshes the list", async () => {
    ...
    expect(createFaq).toHaveBeenCalledWith("tok", { question: "q", answer: "a", category: "c" });
    expect(result.current.form).toEqual({ question: "", answer: "", category: "" });
  ...
  it("surfaces a friendly error message for non-auth failures", async () => {
    vi.mocked(createFaq).mockRejectedValue(new ApiError("Categoria inválida", 400));
    const { result } = renderHook(() => useFaqAdmin());
    await waitFor(() => expect(result.current.loading).toBe(false));
